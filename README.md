...
- [173-adaptive-chunking-large-document](lessons/173-adaptive-chunking-large-document.md) - Agent 自适应分块与大文档处理（Adaptive Chunking & Large Document Processing）：固定/语义/滑动窗口/自适应四种分块策略对比、代码块不可切断的原子单元设计、Token 预算感知动态调整块大小、Map-Reduce 模式分块各自回答再聚合、OpenClaw Read Tool offset/limit 天然分块读取、Python/TypeScript 双版本实现，信息按需提取而非暴力填充
- [174-entity-extraction-slot-filling](lessons/174-entity-extraction-slot-filling.md) - Agent 实体提取与槽位填充（Entity Extraction & Slot Filling）：从自然语言提取结构化实体、Zod Schema 定义槽位与运行时校验、LLM 工具调用模式只提取确定信息、增量合并不覆盖已有值、渐进追问一次一个、枚举槽位用 Inline Buttons 减少摩擦、回显确认、最大轮次防死循环，让 Agent 优雅处理信息不完整的用户输入
- [175-progressive-code-gen-auto-validation](lessons/175-progressive-code-gen-auto-validation.md) - Agent 渐进式代码生成与自动验证（Progressive Code Generation & Auto-Validation）：生成→执行→捕获错误→自动修复闭环、沙箱安全执行、自动测试生成与运行、质量门控（覆盖率/最大重试/进步检测）、卡住时换策略、Claude Code 工作原理揭秘，Agent 写代码不靠猜，靠跑
- [176-tool-versioning-backward-compatibility](lessons/176-tool-versioning-backward-compatibility.md) - Agent 工具版本控制与向后兼容（Tool Versioning & Backward Compatibility）：SemVer 工具版本策略、并行版本/命名空间/适配器三种模式、deprecated 描述自动提示 LLM 迁移、版本协商按 Agent 版本注入不同工具集、参数迁移中间件自动修复旧字段名、安全迁移四阶段（暗启动→灰度→全量→下线）、废弃调用量监控，工具 API 变更不再让 Agent 崩掉
- [177-tool-parallelism-safety-atomic-ops](lessons/177-tool-parallelism-safety-atomic-ops.md) - Agent 工具执行并行安全与原子操作（Tool Execution Parallelism Safety & Atomic Operations）：副作用声明（reads/writes/external）驱动并发分析、拓扑波次调度自动分组、Saga 原子组+补偿回滚、三种失败策略（fail-fast/best-effort/critical-path）、pi-mono ParallelToolExecutor 实战、OpenClaw 工具注册副作用元数据，并发提速但正确性不妥协
- [178-working-memory-attention-management](lessons/178-working-memory-attention-management.md) - Agent 工作记忆与注意力管理（Working Memory & Attention Management）：认知科学四槽模型（System/TaskState/Recent/Episodic/ScratchPad）、按意图动态调整 slot 权重（写代码 vs 闲聊权重完全不同）、工具结果主动遗忘（LRU + 时间衰减）、pi-mono Focus Stack 子任务 context 隔离、OpenClaw Heartbeat 工作记忆使用率监控，context 不是越多越好，聚焦才能让 LLM 发挥最大效能

- [179-latency-budget-allocation-timeout-cascading](lessons/179-latency-budget-allocation-timeout-cascading.md) - Agent 延迟预算分配与超时级联（Latency Budget Allocation & Timeout Cascading）：总预算 = E2E SLO，随执行进度向下游级联动态分配；优先级系数控制各工具占比；critical 工具不可跳过、low 工具预算不足直接 skip；超时三种结果（成功/降级/跳过）；AbortSignal 级联取消；Prometheus 预算利用率监控，让整条工具链的延迟始终可控
- [180-streaming-tool-result-subscription](lessons/180-streaming-tool-result-subscription.md) - Agent 实时流式工具结果订阅（Real-time Streaming Tool Result Subscription）：工具调用从 request-response 升级为 AsyncGenerator push 模式；ToolChunk 统一类型（data/progress/error/done）；tail_logs/exec_stream/poll_until 三个实战工具；onChunk 回调实时转发用户侧体验；Agent Loop 改造支持流式与普通工具混用；对应 OpenClaw exec(yieldMs) 近似实现；流式 vs 轮询选型指南，让 Agent 不再等结果，而是看结果流入
- [181-tool-parameter-auto-completion](lessons/181-tool-parameter-auto-completion.md) - Agent 工具参数智能补全与上下文推断（Tool Parameter Auto-Completion & Contextual Inference）：工具分发层多来源按优先级自动补全缺失参数（会话 Meta/前序工具结果/对话历史/工具默认值）；LLM 显式参数永远最高优先级；类型强转防止字符串/数字混淆；x-from-tool/x-from-path Schema 扩展标注让推断意图显式化；可观测性记录 autoFilled 字段；工具调用重试次数从 2.1 降至 0.3，每次工具调用 token 省 47%
- [182-smart-pagination-cursor-management](lessons/182-smart-pagination-cursor-management.md) - Agent 智能分页与游标管理（Smart Pagination & Cursor Management）：Cursor-based vs Offset-based 分页选型；PaginationMiddleware 透明包装任意 list 工具；Lazy 流式翻页找到即停；Token 感知动态 pageSize；游标 Checkpoint 长任务断点续传；工具 Schema 明确声明分页能力引导 LLM；OpenClaw Read 工具 offset/limit 天然分页哲学，翻页要有策略，找到目标及时停止
- [183-input-preprocessing-pipeline](lessons/183-input-preprocessing-pipeline.md) - Agent 输入预处理管道（Input Preprocessing Pipeline）：用户输入进 LLM 前的六阶段流水线——规范化（Unicode/乱码修复）、语言检测（自动设响应语言）、内容过滤（Prompt Injection 检测）、Token 预算检查（超长智能截断）、上下文增强（时间/用户偏好注入）、意图预分类（用 Haiku 快速路由）；纯函数设计、失败降级不阻断、可观测性完整；输入质量决定输出质量，最高 ROI 的优化
...
- [187-dynamic-few-shot-selection](lessons/187-dynamic-few-shot-selection.md) - Agent 动态 Few-Shot 示例选择（Dynamic Few-Shot Example Selection）：与其手写固定示例，不如从示例库用语义相似度实时检索最相关的 Few-Shot；向量化示例库 + Top-K 检索 + Token 预算动态 K 值；示例质量过滤（成功率/反馈评分）；A/B 测试对比固定 vs 动态效果；TypeScript/Python 双版本；越用越聪明的自适应示例系统
- [188-state-snapshot-time-travel-debugging](lessons/188-state-snapshot-time-travel-debugging.md) - Agent 状态快照与时间旅行调试（State Snapshot & Time-Travel Debugging）：Agent bug 往往在第 17 步才爆发；SnapshotManager 每步保存完整状态（messages/toolResults/variables）；从任意快照恢复重放、创建分支测试不同决策路径、Diff 对比快速定位 bug 起点；危险工具执行前必存快照；自适应快照策略（调试全量/生产精简）；OpenClaw exec+checkpoint 组合实战；把不可复现的 bug 变成可调试的问题
- [189-natural-language-policy-engine](lessons/189-natural-language-policy-engine.md) - Agent 自然语言策略引擎（Natural Language Policy Engine）：用自然语言声明行为约束（"不要夜间发营销邮件"/"写前先读"），Haiku 实时检查，中间件透明注入 Agent Loop，支持热更新无需重部署；Fail-Open/Close 选型；策略缓存降成本；OpenClaw SOUL.md 即此模式最佳落地
- [190-pre-execution-planning-cost-estimation](lessons/190-pre-execution-planning-cost-estimation.md) - Agent 预执行计划与代价估算（Pre-execution Planning & Cost Estimation）：像 SQL EXPLAIN 一样执行前先规划；Haiku 生成工具调用计划、工具代价元数据注入成本估算、并行波次优化（拓扑排序节省 40%+ 延迟）、阈值驱动人工确认、计划注入 System Prompt 防执行漂移；pi-mono TaskSystem/OpenClaw Cron 大任务前自动汇报；执行前先思考，思考前先估算
- [191-functional-core-imperative-shell](lessons/191-functional-core-imperative-shell.md) - Agent 函数式核心、命令式外壳（Functional Core, Imperative Shell）：决策逻辑写成纯函数（state + observation → action），所有 I/O 推到外壳层；测试无需 mock、无需网络、毫秒级；TypeScript/Python 双版本；OpenClaw SOUL.md/Skills/HEARTBEAT = 纯函数核心，runtime = 命令式外壳；重构不再牵一发动全身，bug 本地直接复现
- [192-mcp-server-implementation](lessons/192-mcp-server-implementation.md) - Agent MCP 服务端实现（MCP Server Implementation）：把自己的工具暴露为标准协议，一次实现所有 MCP 客户端（Claude Desktop/OpenClaw/Cursor）都能用；Tools/Resources/Prompts 三种能力；stdio vs HTTP+SSE 两种传输；TypeScript SDK + Python FastMCP 双语实现；Schema 质量决定 LLM 使用效果；isError vs throw 错误分层；mcporter + MCP Inspector 调试；Skills 与 MCP Server 选型对比
- [193-progressive-autonomy-trust-scoring](lessons/193-progressive-autonomy-trust-scoring.md) - Agent 渐进式自治与信任度量（Progressive Autonomy & Trust Scoring）：Agent 通过行为记录动态赚取/失去自治权；四级信任矩阵（SUPERVISED/ASSISTED/AUTONOMOUS/TRUSTED）× 五类操作类型；指数时间衰减 + 失败惩罚非对称；allow_and_report 高信任异步上报；Telegram Inline Buttons 人工审批；OpenClaw Proxy 中间件零侵入注入；信任不是配置出来的，是赚来的
- [194-multi-env-config-management](lessons/194-multi-env-config-management.md) - Agent 多环境配置管理（Multi-Environment Configuration Management）：Base→Env Overlay→Runtime Patch 三层叠加，Zod 启动校验快速失败；工具 Mock 注册表让 Dev 零副作用；环境感知中间件自动切换超时/重试/审批策略；CI 配置合法性检查；OpenClaw SOUL.md/USER.md/HEARTBEAT.md 即此模式的文本落地，配置错误必须在启动时爆，不能等凌晨三点
- [195-self-consistency-checking](lessons/195-self-consistency-checking.md) - Agent 自我一致性检验（Self-Consistency Checking）：同一问题多路采样 LLM + 多数投票取最稳定答案；extractAnswer 归一化 + majorityVote 置信度量化；按风险等级动态决定采样次数（N=1/3/5）；便宜模型采样节省成本；工具调用前高风险决策检验；OpenClaw 子 Agent 并发投票；把"一个人的判断"升级为"陪审团裁决"，幻觉自然消散
- [196-dual-process-decision-architecture](lessons/196-dual-process-decision-architecture.md) - Agent 双系统决策架构（Dual-Process Decision Architecture）：快系统（System 1：Haiku + 无 thinking，直觉反射）+ 慢系统（System 2：Sonnet + extended thinking，深度推理）；规则优先 + Haiku 分类器路由；不可逆操作强制 System 2；三档配置（SIMPLE/MODERATE/COMPLEX）；成本节省 68%、简单问答延迟降 85%；OpenClaw /reasoning 自适应模式；TypeScript/Python 双版本；好的 Agent 不是总在全力思考，而是知道什么时候该努力思考
- [197-tool-call-chain-visualization](lessons/197-tool-call-chain-visualization.md) - Agent 工具调用链路可视化与 Debug 热图（Tool Call Chain Visualization & Debug Heatmap）：透明中间件自动采集 ToolCallTrace；Mermaid 序列图生成调用时序；ASCII 火焰图终端直出零依赖；工具热图（调用频率 × 平均耗时 × 错误率）定位瓶颈；OpenClaw session_history 作为天然数据源；Cron 定时生成性能报告；Python 装饰器版本简洁实现；调用链路从黑盒变玻璃盒，热点瓶颈秒级定位
- [198-semantic-deduplication-knowledge-merging](lessons/198-semantic-deduplication-knowledge-merging.md) - Agent 语义去重与知识合并（Semantic Deduplication & Knowledge Merging）：多工具结果注入 LLM 前去重合并；Jaccard 字面去重（零成本）+ 向量语义去重（高精度）；LLM 驱动多源知识合并；按置信度保留最优版本；知识管道与 Agent Loop 集成；节省 80% token，信噪比大幅提升；RAG 场景去重实战
- [199-agent-anti-patterns-common-pitfalls](lessons/199-agent-anti-patterns-common-pitfalls.md) - Agent 反模式与常见陷阱（Anti-Patterns & Common Pitfalls）：10 大 Agent 反模式系统梳理——上帝工具/Context 垃圾填埋场/工具爆炸/同步阻塞/无界重试/Prompt 意大利面/静默吞异常/无状态假设/过度智能/裸奔部署；每个反模式配症状+根因+正确做法；运行时反模式检测中间件实战；10 条部署前检查清单；好的 Agent 不在于能做多少，而在于能避开多少坑
- [200-context-anchoring-information-pinning](lessons/200-context-anchoring-information-pinning.md) - Agent 上下文锚点与关键信息钉（Context Anchoring & Information Pinning）：解决长对话中关键约束被 LLM "遗忘"的问题；ContextAnchor 数据结构 + Haiku 自动提取 + 中间件注入；priority 分级（critical/high/medium）+ reinjectEvery 控制注入频率；XML 标签格式增强 Claude 注意力；TTL 过期机制防止锚点堆积；OpenClaw SOUL.md 即静态锚点的最佳落地；Context Anchoring = 给 LLM 贴便利贴，让真正重要的约束始终保持显著
- [201-multilingual-support-i18n](lessons/201-multilingual-support-i18n.md) - Agent 多语言支持与 i18n（Multilingual Support & Internationalization）：规则检测（CJK/ASCII 比例）+ Haiku 精准检测二级策略；SessionLocale 管理语言状态；locale 感知的 system prompt 注入；i18n 消息模板替换硬编码字符串；工具输出本地化（跳过结构化数据）；用户显式覆盖优先；OpenClaw SOUL.md/USER.md 三层语言策略；TypeScript/Python 双版本；语言是会话状态，不是 LLM 能力
- [202-slo-sla-error-budget](lessons/202-slo-sla-error-budget.md) - Agent SLO/SLA 管理与错误预算（Error Budget & SLO Management）：SLI/SLO/Error Budget 三层概念；Agent 特有 SLI 设计（任务完成率/幻觉率/工具调用成功率）；双窗口 Burn Rate 告警；Error Budget 驱动发布门控；Cron 每日 SLO 报告；SLO 是承诺，Error Budget 是创新的底气
- [203-proactive-assistance-intent-prediction](lessons/203-proactive-assistance-intent-prediction.md) - Agent 主动助手与意图预测（Proactive Assistance & Intent Prediction）：三层主动能力模型（需求感知/意图预测/主动推送）、意图转移矩阵预取数据、NeedDetector 信号检测、PushThrottler 节奏控制防打扰、OpenClaw Heartbeat + SOUL.md 作为主动助手完整基础设施；好的 Agent 在你开口之前就已经在路上了
- [204-agent-as-a-service](lessons/204-agent-as-a-service.md) - Agent 作为 REST API 服务（Agent-as-a-Service）：把 Agent 包装成标准 HTTP 服务；同步模式（< 30s 直接返回结果）vs 异步模式（立即返回 job_id + 轮询/Webhook）；API Key 身份验证 + 限流中间件；Job Store + TTL 自动清理；Webhook 回调 + HMAC 签名验证；与 MCP Server/Webhook-Driven/Subagents 的选型对比；10 条生产部署检查清单；让任何前端/移动端/后端系统都能一行 HTTP 调用你的 Agent
...
- [212-error-message-engineering](lessons/212-error-message-engineering.md) - Agent 错误消息工程化（Error Message Engineering）：工具错误消息不是给开发者看的，是给 LLM 看的；错误三段式（发生了什么/为什么/怎么修）；可恢复性四级分类（RETRY_WITH_FIX/RETRY_AFTER_WAIT/ESCALATE_TO_USER/ABORT_AND_REROUTE）；StructuredError 统一信封；错误模板系统（SQL/文件/限流/权限）；工具层错误转换中间件；常见错误自动 hint 映射；TypeScript/Python 双版本；7 项错误消息设计 Checklist；好错误消息让 Agent 自愈，烂错误消息让 Agent 原地打转
- [213-tool-call-graph-hot-path-optimization](lessons/213-tool-call-graph-hot-path-optimization.md) - Agent 工具调用图谱与热路径优化（Tool Call Graph & Hot Path Optimization）：透明中间件录制工具调用序列、N-gram 建有向图、热路径分析（调用频率×延迟×节点重要性）、自动生成复合工具把 3 步热路径合并为 1 次 LLM round-trip、Mermaid 可视化调用图、OpenClaw session_history 作数据源；生产实测延迟降低 62%，LLM 调用次数降低 38%；TypeScript/Python 双版本
- [214-realtime-user-feedback-online-learning](lessons/214-realtime-user-feedback-online-learning.md) - Agent 实时用户反馈与在线学习（Real-time User Feedback & Online Learning）：显式（👍👎/纠正）+ 隐式（重复查询/提前放弃）双信号捕获；FeedbackMiddleware 动态调整 system prompt；Redis 跨会话持久化；OpenClaw Telegram 反应直接驱动 prompt 调整；memory/feedback-profile.json = 轻量在线学习；pi-mono responseProcessor hook 集成；在线学习 vs 离线微调选型对比；不需要重训练，一个 JSON + 一段 middleware，Agent 越用越懂你
- [215-multi-step-structured-data-collection](lessons/215-multi-step-structured-data-collection.md) - Agent 多步骤结构化数据收集（Multi-Step Structured Data Collection）：一次一问替代传统多字段表单；FormSchema（字段定义/验证规则/条件跳转）+ FormEngine 核心引擎；skipIf 动态跳过不相关字段 + prefill 自动预填已知数据；LLM 自然语言解析 select 选项（用户不需要精确输入）；断点续传（memory/forms/ 持久化进度）；随时取消并恢复正常对话；TypeScript/Python 双版本；Telegram inline buttons 作 select 快捷键；表单 + 自由对话无缝协作模式
- [216-artifact-store-file-lifecycle](lessons/216-artifact-store-file-lifecycle.md) - Agent Artifact Store 与文件生命周期管理：把报表/图片/PDF/补丁等交付物从本地路径升级为可审计、可权限控制、可自动清理的 Artifact；artifact_id 引用传递、signed URL、TTL GC、sha256 防篡改、渠道附件适配；learn-claude-code 本地索引版 + pi-mono Storage Adapter 生产版 + OpenClaw AUTO_DELIVER/NO_REPLY 模式；LLM 只负责理解和决策，文件后勤交给基础设施
- [217-capability-discovery-negotiation](lessons/217-capability-discovery-negotiation.md) - Agent 能力探测与协商（Capability Discovery & Negotiation）：运行时检测渠道/工具/权限/上下文能力，按平台能力矩阵过滤工具、适配输出、选择审批模式；避免把不可用工具暴露给 LLM；learn-claude-code 最小能力表 + pi-mono CapabilityProvider 生产版 + OpenClaw channel capabilities 实战；好的 Agent 不是假设自己什么都能做，而是知道当前环境能做什么，并能优雅降级
- [218-interrupt-handling-task-pivot](lessons/218-interrupt-handling-task-pivot.md) - Agent 中断处理与任务改道（Interrupt Handling & Task Pivoting）：用户说“停/暂停/改目标”时，Agent 要能安全刹车；InterruptSignal 控制通道、AbortSignal 贯穿 LLM/工具/Sub-agent、Checkpoint 保存进度、Pivot 只复用事实不复用旧结论、外部副作用前最后检查一次中断状态；learn-claude-code asyncio.Event 教学版 + pi-mono TaskRunController 生产版 + OpenClaw 长任务句柄实战；没有刹车的 Agent 只适合 Demo，有中断处理的 Agent 才适合生产
- [219-task-lease-heartbeat-renewal](lessons/219-task-lease-heartbeat-renewal.md) - Agent 任务租约与心跳续约（Task Lease & Heartbeat Renewal）：给长任务加 leaseOwner/leaseToken/leaseExpiresAt，执行中 heartbeat 续约，过期后安全接管；fencing token 防旧 worker 复活乱写，checkpoint 支持断点续跑，外部副作用用 idempotencyKey 防重复；learn-claude-code SQLite 教学版 + pi-mono TaskLeaseManager 生产版 + OpenClaw cron/sub-agent 长任务实战；长任务不是靠进程一直活着完成，而是靠 lease、heartbeat、checkpoint 和幂等副作用完成
- [220-dead-letter-queue-failed-task-recovery](lessons/220-dead-letter-queue-failed-task-recovery.md) - Agent 死信队列与失败任务恢复（Dead Letter Queue & Failed Task Recovery）：Retry 负责短暂失败自动恢复，DLQ 负责重试耗尽/不可恢复任务的可观测、可复盘、可人工介入闭环；结构化失败档案（attempts/error/history/checkpoint/replayableInput/idempotencyKey）、失败分类（transient/rate_limited/correctable/poison）、learn-claude-code SQLite 教学版 + pi-mono FailedTaskManager 生产版 + OpenClaw cron/sub-agent 文件式 DLQ 实战；失败路径也是一等公民
- [221-notification-routing-noise-budget](lessons/221-notification-routing-noise-budget.md) - Agent 通知路由与噪声预算（Notification Routing & Noise Budget）：主动助手不是有事就刷屏，而是按 priority/dedupe/quiet hours/noise budget/digest 做通知治理；critical/high 才立即打断，normal 进入摘要，low 只写日志；learn-claude-code 文件状态版 + pi-mono NotificationPolicy 生产版 + OpenClaw Heartbeat 实战；好 Agent 懂什么时候闭嘴，什么时候必须开口
- [222-answer-grounding-citation-guard](lessons/222-answer-grounding-citation-guard.md) - Agent 答案溯源与引用校验（Answer Grounding & Citation Guard）：把工具结果/文档片段登记为 Evidence Source，最终回答拆成 Claim 并绑定 citation，输出前校验每个关键断言是否被证据支持；learn-claude-code CitationGuard 教学版 + pi-mono EvidenceStore/中间件生产版 + OpenClaw 工具证据实战；没有证据的结论默认只是猜测
- [223-session-state-migration-compatibility](lessons/223-session-state-migration-compatibility.md) - Agent 会话状态迁移与兼容升级（Session State Migration & Compatibility Upgrade）：给持久化状态加 schemaVersion，读取入口统一跑 migration registry，迁移后 schema validate，失败进 DLQ；learn-claude-code Python 教学版 + pi-mono Zod/StateStore 生产版 + OpenClaw 文件记忆版本化实战；长会话升级不是部署新代码，而是带着旧记忆安全醒来
- [224-runbook-automation-safe-operations](lessons/224-runbook-automation-safe-operations.md) - Agent 运行手册自动化与安全执行（Runbook Automation & Safe Operations）：把人类 SOP 变成可验证、可审批、可回滚的操作图；Runbook = DAG + Policy + Evidence，先 read-only 诊断，再 dry-run 计划，高风险步骤走 approval，执行后 verify；learn-claude-code Python dict 教学版 + pi-mono 强类型 DSL/Policy Engine 生产版 + OpenClaw Cron/Heartbeat 运维实战；LLM 是 planner，不是 root shell
- [225-config-drift-reconciliation](lessons/225-config-drift-reconciliation.md) - Agent 配置漂移检测与自动修复（Configuration Drift Detection & Reconciliation）：持续对比 desired state 与 actual state，先 normalizer/ignore rules 去噪，再 severity rules 判断风险；warning 且 autoFixable 的漂移可幂等 patch，danger 字段升级人工；learn-claude-code Python reconcile loop 教学版 + pi-mono DriftDetector/Reconciler 生产版 + OpenClaw Heartbeat/Cron 实战；Agent 不只是执行命令，还要持续判断现实世界是否还符合期望状态
- [226-data-freshness-staleness-management](lessons/226-data-freshness-staleness-management.md) - Agent 数据新鲜度与过期管理（Data Freshness & Staleness Management）：上下文里的观察不是永恒事实；给工具结果加 observedAt/maxAge/stalePolicy，用户问“最新/现在/实时”时强制 refresh，副作用执行前二次检查 critical observations；learn-claude-code FreshnessGuard 教学版 + pi-mono FreshnessPolicy/中间件生产版 + OpenClaw Heartbeat/Cron 实战；旧数据做新决策，是生产 Agent 最隐蔽的坑
- [227-runtime-fingerprint-reproducible-runs](lessons/227-runtime-fingerprint-reproducible-runs.md) - Agent 运行时指纹与可复现执行（Runtime Fingerprint & Reproducible Runs）：每次 run 记录模型、prompt hash、工具 schema hash、配置 hash、gitSha、渠道能力和 memory snapshot；RunManifest 先于 LLM 调用写入，复现 CLI 对比原环境与当前环境差异；learn-claude-code RuntimeFingerprint 教学版 + pi-mono RunManifest 中间件 + OpenClaw cron/session_status 实战；没有运行时指纹的 Agent bug 只能靠玄学复盘，有指纹才有工程化复现
- [228-tool-argument-canonicalization](lessons/228-tool-argument-canonicalization.md) - Agent 工具参数规范化与执行前校验（Tool Argument Canonicalization & Preflight Validation）：LLM raw args 不能直接执行，先 canonicalize 相对时间/路径/枚举/单位，再 schema validate、权限校验、风险预检和审计；learn-claude-code ToolArgGuard 教学版 + pi-mono ToolDefinition/Dispatcher 生产版 + OpenClaw 工具调用实战；LLM 可以模糊，工具不能模糊。
- [229-effect-journal-two-phase-commit](lessons/229-effect-journal-two-phase-commit.md) - Agent 副作用日记与两阶段提交（Effect Journal & Two-Phase Commit）：外部写操作先写 Effect Journal，再 commit，再 verify；用 argsHash/idempotencyKey/externalId/status 把发消息、创建 PR、部署等副作用变成可恢复状态机；learn-claude-code 文件式账本教学版 + pi-mono 工具分发中间件生产版 + OpenClaw message/git/deploy 实战；没有账本的 Agent 靠运气执行，有账本的 Agent 才能放心自动化。
- [230-memory-write-policy-pollution-guard](lessons/230-memory-write-policy-pollution-guard.md) - Agent 记忆写入策略与污染防护（Memory Write Policy & Pollution Guard）：记忆写入是特权操作；候选记忆先按 preference/fact/decision/instruction 分类，携带 source/authority/evidence，再由策略决定 promote/inbox/reject；learn-claude-code Memory Inbox 教学版 + pi-mono MemoryWriteMiddleware 生产版 + OpenClaw MEMORY.md 实战；长期记忆不是垃圾桶，防污染比会记住更重要。
- [231-priority-inheritance-inversion-avoidance](lessons/231-priority-inheritance-inversion-avoidance.md) - Agent 任务优先级继承与防优先级反转（Priority Inheritance & Inversion Avoidance）：高优先级任务等待低优先级依赖时，低优先级任务临时继承高优先级，优先释放资源锁/关键路径；basePriority/effectivePriority 双字段、依赖链 boost、TTL/reason/cycle detection 三保护；learn-claude-code Python 调度器 + pi-mono QueueMiddleware + OpenClaw Cron/Sub-agent/资源锁实战；调度不是只看谁重要，还要看谁挡住了重要任务。
- [232-schema-registry-context-contracts](lessons/232-schema-registry-context-contracts.md) - Agent Schema Registry 与上下文契约（Schema Registry & Context Contracts）：把工具结果、记忆、任务状态、子 Agent 交接都当成隐形 API 管理；schema/version/parse/migrate/llmContract 五件套；写入前校验、读取时迁移、注入 prompt 时只放简短契约；learn-claude-code Python Registry + pi-mono Zod 中间件 + OpenClaw 文件记忆 schema 头实战；上下文不是草稿纸，是跨时间流动的 API。
- [233-completion-gates-done-criteria](lessons/233-completion-gates-done-criteria.md) - Agent 完成判定与验收闸门（Completion Gates & Done Criteria）：把“差不多完成了”变成可执行验收；doneCriteria + required gates + evidence 三件套；最终回复前强制跑 lesson file/README/TOOLS/messageId/git push 等闸门；learn-claude-code Python GateRunner + pi-mono CompletionGate 中间件 + OpenClaw Cron 验收脚本实战；成熟 Agent 的完成不是感觉，是证据。
- [234-change-risk-scoring-release-gates](lessons/234-change-risk-scoring-release-gates.md) - Agent 变更风险评分与发布门控（Change Risk Scoring & Release Gates）：把 diff、敏感文件、外部副作用、测试证据和历史失败转成风险分数，再自动决定 PR/review/canary/manual_only 发布路径；learn-claude-code Python 风险评分器 + pi-mono RiskPolicy/Gate Router + OpenClaw PR 风险摘要实战；成熟 Agent 不是永远大胆自动化，而是知道什么时候该减速。
- [235-tool-result-confidence-degradation](lessons/235-tool-result-confidence-degradation.md) - Agent 工具结果置信度与降级决策（Tool Result Confidence & Degradation Decisions）：工具成功不等于结果可信；给每个工具结果加 confidence/completeness/caveats/suggestedAction，Agent Loop 据此自动 retry/fallback/ask/abort；learn-claude-code 评分器 + pi-mono ConfidenceMiddleware + OpenClaw 工具证据实战；成熟 Agent 不只会查资料，还知道证据够不够支撑动作。
- [236-outbox-delivery-receipts](lessons/236-outbox-delivery-receipts.md) - Agent Outbox 与投递回执（Outbox & Delivery Receipts）：把发消息/邮件/Webhook 从一次性副作用升级成可靠投递流水线；先写 Outbox，再由 Dispatcher 发送，保存 provider message id 回执；idempotencyKey 防重复，lease claim 防并发，按 429/5xx/400/403 分类重试或 dead-letter；learn-claude-code SQLite 教学版 + pi-mono OutboxMiddleware/Dispatcher 生产版 + OpenClaw 课程 cron 实战；可靠 Agent 的最后一步不是“应该发了”，而是有证据可恢复。
- [237-inbox-command-bus](lessons/237-inbox-command-bus.md) - Agent 收件箱与命令总线（Inbox & Command Bus）：所有 Telegram/Webhook/Cron/GitHub 入口先写 Inbox，再由 Command Bus 统一路由；dedupeKey 防重复、priority 支持插队、pending/processing/done/dead_letter 状态可恢复；learn-claude-code SQLite 教学版 + pi-mono InboxAdapter/CommandBus 生产版 + OpenClaw cron/Telegram 实战；Outbox 让 Agent 说出去可靠，Inbox 让 Agent 听进来可靠。
- [238-human-override-pause-resume](lessons/238-human-override-pause-resume.md) - Agent 人工接管与暂停恢复（Human Override & Pause/Resume Control）：给 Always-on Agent 增加可审计控制面；ControlState 用 mode/revision/operator/reason 表达暂停、人工接管、取消与恢复；Agent Loop 和外部副作用前二次检查控制状态，暂停时写 checkpoint，恢复时携带 operatorInstruction 重新验证关键观察；learn-claude-code JSON 控制文件教学版 + pi-mono ControlMiddleware 生产版 + OpenClaw cron/sub-agent 实战；成熟 Agent 的自主性，是该跑时能跑、该让位时立刻让位。
- [239-secrets-boundary-credential-leasing](lessons/239-secrets-boundary-credential-leasing.md) - Agent 密钥边界与短期凭证租赁（Secrets Boundary & Credential Leasing）：LLM 不应该知道密钥，只应请求受限、短期、可审计的能力；SecretRef 进上下文，secret value 只在工具执行层局部解析；CredentialLease 绑定 runId/toolName/TTL/maxUses，防跨任务复用；learn-claude-code Python SecretBoundary + pi-mono SecretLeaseMiddleware + OpenClaw TOOLS.md 分级实战；把“知道密码”变成“被授权执行一次能力”。
- [240-data-access-scope-row-level-auth](lessons/240-data-access-scope-row-level-auth.md) - Agent 数据访问作用域与行级授权（Data Access Scope & Row-Level Authorization）：每次工具调用都带 actor/tenant/scope/purpose，LLM 只表达意图，系统在 Tool Middleware 与 Repository 层强制权限和 tenant 过滤；learn-claude-code AccessScope 教学版 + pi-mono withAccessControl 中间件 + OpenClaw 群聊/私聊/Cron 权限实战；数据安全不能靠模型乖，要靠每一层都不信任上一层。
- [241-evidence-conflict-resolution](lessons/241-evidence-conflict-resolution.md) - Agent 证据冲突处理与事实仲裁（Evidence Conflict Resolution & Fact Arbitration）：把工具结果、记忆、缓存和用户输入统一成 Evidence，按来源可信度、新鲜度、直接性和风险等级处理冲突；learn-claude-code Python 仲裁器 + pi-mono FactStore/ConflictPolicy + OpenClaw push 前 live check 实战；靠谱 Agent 不是永远有答案，而是知道什么时候证据不够、必须重新查。
- [242-observation-snapshot-pre-action-revalidation](lessons/242-observation-snapshot-pre-action-revalidation.md) - Agent 观察快照与执行前复核（Observation Snapshot & Pre-Action Revalidation）：把关键观察冻结为 Snapshot，副作用工具执行前按风险等级重新验证前提是否仍成立；learn-claude-code Python Revalidator + pi-mono Tool Middleware + OpenClaw git push/发群消息前 live check 实战；成熟 Agent 不是相信几分钟前的世界，而是在动手前确认现实没变。
- [243-state-invariants-consistency-assertions](lessons/243-state-invariants-consistency-assertions.md) - Agent 状态不变量与一致性断言（State Invariants & Consistency Assertions）：把“绝不能坏”的系统规则写成可执行断言，在状态写入后、外部副作用前、最终回复前自动检查；learn-claude-code InvariantGuard + pi-mono InvariantMiddleware + OpenClaw 课程 Cron 验收闸门实战；成熟 Agent 不只会做事，还要证明自己没有把状态做坏。
- [244-dependency-inventory-supply-chain-guard](lessons/244-dependency-inventory-supply-chain-guard.md) - Agent 依赖清单与供应链护栏（Dependency Inventory & Supply-Chain Guard）：Agent 在安装依赖、跑脚本、审计仓库前先建立 manifest/lockfile/scripts/workflows 的执行面清单，识别 postinstall、curl|bash、未 pin Actions、许可证和漏洞风险；learn-claude-code Python 扫描器 + pi-mono SupplyChainGate + OpenClaw GitHub 审计实战；会写代码的 Agent 很有用，会在安装前停一下的 Agent 才可靠。
- [245-plan-signing-approval-integrity](lessons/245-plan-signing-approval-integrity.md) - Agent 执行计划签名与审批防篡改（Plan Signing & Approval Integrity）：人工审批必须绑定不可变执行计划，而不是一句模糊确认；canonical plan hash 绑定工具名、参数、目标环境、风险、证据和执行主体；执行前重新计算 hash，计划或命令变化就强制重新审批；learn-claude-code Python PlanSigner + pi-mono ApprovalMiddleware + OpenClaw native approval 实战；成熟 Agent 的审批不是点按钮，是确认一份不可篡改的副作用契约。
- [246-env-drift-config-fingerprinting](lessons/246-env-drift-config-fingerprinting.md) - Agent 环境漂移检测与配置指纹（Environment Drift Detection & Config Fingerprinting）：把 git sha、lockfile、运行时版本、工具 schema、脱敏配置和 secret version 汇总成可比较 fingerprint；计划/审批绑定 fingerprint，执行高风险副作用前重新采样，发现漂移就阻断并要求复核；learn-claude-code Python EnvFingerprint + pi-mono EnvDriftMiddleware + OpenClaw git push/发群消息前 live check 实战；成熟 Agent 要证明“刚才检查的世界”和“现在动手的世界”还是同一个世界。
- [247-delegation-tokens-subtask-capability-chains](lessons/247-delegation-tokens-subtask-capability-chains.md) - Agent 委托令牌与子任务权限链（Delegation Tokens & Subtask Capability Chains）：子 Agent 默认无权限，必须通过短期 DelegationToken 显式授权；child scopes/tools 必须是 parent 子集，绑定 parentRunId/childRunId/TTL/nonce/signature；Tool Dispatcher 强制 allowedTools/readonly/allowedPaths/maxToolCalls；learn-claude-code Python 委托校验器 + pi-mono DelegationMiddleware + OpenClaw Sub-agent 派活实战；成熟 Agent 的委托不是“相信它听话”，而是“它越界也做不到”。
- [248-capability-revocation-session-deauthorization](lessons/248-capability-revocation-session-deauthorization.md) - Agent 能力吊销与会话撤权（Capability Revocation & Session Deauthorization）：授权不是发出去就永远有效；Capability 带 jti/sessionId/actor/scopes/expiresAt/version，Revocation Registry 支持按 token/session/actor/scope/version 吊销；Tool Dispatcher 在每次副作用前强制检查，用户取消或风险升高时立即让旧能力失效；learn-claude-code Python Registry + pi-mono Tool Middleware + OpenClaw sub-agent/长任务撤权实战；成熟 Agent 不只会授权，还要能立刻收回授权。
- [249-policy-as-code-tool-governance](lessons/249-policy-as-code-tool-governance.md) - Agent Policy-as-Code 与工具治理（Policy-as-Code & Tool Governance）：把工具权限、风险分级、审批要求、dry-run 默认策略从 prompt 移到可测试策略引擎；PolicyDecision 返回 allow/deny/require_approval/dry_run 和 reasons/constraints；learn-claude-code Python 纯函数策略 + pi-mono Policy Middleware + OpenClaw 渠道能力/审批/记忆边界实战；成熟 Agent 不是靠模型自觉守规矩，而是工具层只执行 Policy 允许的事。
- [250-intent-ledger-action-accountability](lessons/250-intent-ledger-action-accountability.md) - Agent 意图账本与行动可追责（Intent Ledger & Action Accountability）：把用户请求、标准化意图、作用域、计划动作、工具调用和完成证据串成一条可审计链路；learn-claude-code JSONL 账本 + pi-mono IntentLedgerMiddleware + OpenClaw Cron/Telegram/Git 实战；成熟 Agent 不只要留下“做了什么”的日志，还要证明“为什么应该这么做”。
- [251-cache-invalidation-dependency-tags](lessons/251-cache-invalidation-dependency-tags.md) - Agent 缓存失效与依赖标签（Cache Invalidation & Dependency Tags）：缓存条目绑定依赖 tags，实体变化时精准批量失效；TTL + tag invalidation + high-risk live check 三层模型；learn-claude-code SQLite 教学版 + pi-mono CacheInvalidationMiddleware + OpenClaw Heartbeat/Cron 文件缓存实战；TTL 负责时间到了忘，Tag 负责世界变了立刻忘。
- [252-cache-stampede-stale-revalidate](lessons/252-cache-stampede-stale-revalidate.md) - Agent 缓存击穿与后台刷新（Cache Stampede & Stale-While-Revalidate）：缓存过期/失效后用 SingleFlight 合并重建、TTL jitter 分散过期、soft/hard TTL 支持 stale-while-revalidate、negative cache 防重试风暴；learn-claude-code AsyncSWRCache + pi-mono SwrCacheMiddleware + OpenClaw Heartbeat/Cron 缓存实战；缓存命中是优化，缓存失效才是系统设计的考试。
- [253-cache-warming-hot-key-governance](lessons/253-cache-warming-hot-key-governance.md) - Agent 缓存预热与热点键治理（Cache Warming & Hot Key Governance）：把缓存从被动优化升级为运行时资源管理；启动/定时/预测/发布预热，基于 hit/miss/rebuild_ms/inflight_waiters 识别热点 key，配合 refresh-ahead、延长 soft TTL、重建限流、key_class 指标治理；learn-claude-code CacheWarmer + pi-mono CacheGovernanceMiddleware + OpenClaw Heartbeat/Cron 预热实战；热点不是缓存的奖励，而是缓存系统的压力测试。
- [254-cache-observability-hit-ratio-slo](lessons/254-cache-observability-hit-ratio-slo.md) - Agent 缓存可观测性与命中率 SLO（Cache Observability & Hit Ratio SLO）：缓存不只是 get/set，而是 hit/miss/stale/rebuild_ms/error_rate 的可观测运行时；按 key_class 统计命中率和 P95 重建耗时，低于 SLO 自动预热、延长 soft TTL 或告警；learn-claude-code CacheMetrics + pi-mono ObservedCache + OpenClaw Heartbeat/Cron 巡检实战；没有观测的缓存只是玄学优化。
- [255-cache-consistency-read-write-policies](lessons/255-cache-consistency-read-write-policies.md) - Agent 缓存一致性与读写策略（Cache Consistency & Read-Write Policies）：按风险和读写关系选择 read-through/write-through/write-around/read-your-writes/bypass-cache；高风险副作用前强制 live check，外部写后失效 tags，本 run 内用 overlay 保证读到自己刚写的结果；learn-claude-code PolicyCache 教学版 + pi-mono CacheConsistencyMiddleware + OpenClaw 课程 cron 实战；会用缓存的 Agent 很快，知道什么时候不用缓存的 Agent 才可靠。
- [256-cache-permission-boundaries-tenant-isolation](lessons/256-cache-permission-boundaries-tenant-isolation.md) - Agent 缓存权限边界与租户隔离（Cache Permission Boundaries & Tenant Isolation）：cache key 带 tenant/actor/scope_hash，value envelope 二次校验，敏感工具结果默认不跨 actor 共享；learn-claude-code SecureCache + pi-mono SecureToolCache + OpenClaw scoped file cache 实战；缓存第一目标不是快，而是不能把不该看的数据给错人。
- [257-cache-eviction-lifecycle-policies](lessons/257-cache-eviction-lifecycle-policies.md) - Agent 缓存淘汰与生命周期策略（Cache Eviction & Lifecycle Policies）：缓存不能只会写入，还要按 class/risk/ttl/priority/tags 管生命周期；空间不足时按风险、价值和访问时间淘汰，敏感数据不允许 stale，用户/租户/依赖变化时支持精准 purge；learn-claude-code LifecycleCache + pi-mono CacheLifecyclePolicy/LifecycleCache + OpenClaw 课程 cron 文件式缓存实战；可靠缓存不是永远命中，而是该忘的时候真忘。
- [258-cache-compaction-summarized-entries](lessons/258-cache-compaction-summarized-entries.md) - Agent 缓存压缩与摘要降级（Cache Compaction & Summarized Entries）：大体积工具结果不必只在 raw 和 delete 间二选一；按 size/access/risk 先压缩成 summary+facts+rawPointer，敏感/审计数据禁止 LLM 摘要，精确回答再回源复核；learn-claude-code CompactingCache + pi-mono CompactingCache 中间件 + OpenClaw 文件式缓存实战；成熟缓存会把大上下文压成可复查的知识索引。
- [259-cache-poisoning-defense-trust-zones](lessons/259-cache-poisoning-defense-trust-zones.md) - Agent 缓存投毒防御与信任分区（Cache Poisoning Defense & Trust Zones）：缓存条目不是天然可信，必须记录 source/trust/validation/provenance；按 trusted/verified/untrusted/derived 分区，读取时根据 answer/planning/external_side_effect/security_decision 做用途限制；learn-claude-code CachePoisoningGuard + pi-mono TrustedCache 中间件 + OpenClaw 群聊/网页/文件缓存实战；缓存命中不等于可信，能不能用要看来源和用途。
